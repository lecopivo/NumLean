...
/-- Scalar specialization of `eval_lt`, matching the bound goals generated by `GetElem` on
flat tensor layouts. -/
theorem eval_toScalar_lt {n : Nat} (f : FinHTupleMap src h(n)) (i : HTuple Nat p)
    (hi : i <ₑ src) : (f i : HTuple Nat .leaf).toScalar < n := by
  simpa using f.eval_lt i hi

/-- Scalar layout bound from a zero-origin range hypothesis. -/
theorem eval_toScalar_lt_of_mem {n : Nat} (f : FinHTupleMap src h(n)) (i : HTuple Nat p)
    (hi : i ∈ ((0 : HTuple Nat p)...src)) : (f i : HTuple Nat .leaf).toScalar < n := by
  exact f.eval_toScalar_lt i (HTuple.Range.mem_iff_le_lt.mp hi).2

/-- Contradiction form of `eval_toScalar_lt_of_mem`, matching `grind`'s negated `<` goals. -/
theorem not_le_toScalar_of_mem {n : Nat} (f : FinHTupleMap src h(n)) (i : HTuple Nat p)
    (hi : i ∈ ((0 : HTuple Nat p)...src))
    (hle : n ≤ (f i : HTuple Nat .leaf).toScalar) : False := by
  exact (Nat.not_le_of_gt (f.eval_toScalar_lt_of_mem i hi)) hle

grind_pattern not_le_toScalar_of_mem =>
  i ∈ ((0 : HTuple Nat p)...src), n ≤ (f i : HTuple Nat .leaf).toScalar
attribute [get_tensor_elem =>] not_le_toScalar_of_mem

macro_rules
  | `(tactic| get_tensor_elem_tactic) =>
      `(tactic|
        first
        | exact FinHTupleMap.eval_toScalar_lt_of_mem _ _ (by assumption)
        | exact FinHTupleMap.eval_toScalar_lt_of_mem _ _
            (HTuple.Range.mem_zero_prod (by assumption) (by assumption))
        | get_tensor_elem_tactic_base)
...
@[grind ←]
theorem injective_strided1D {len n : Nat} (off inc : Nat)
    (hb : off + len * inc ≤ n ∧ inc ≠ 0) :
    (strided1D (len := len) (n := n) off inc hb).Injective := by
  intro ⟨.leaf i, hi⟩ ⟨.leaf j, hj⟩ h
  apply FinHTuple.ext
  have hval := congrArg FinHTuple.val h
  simp [evalFin, eval_strided1D] at hval ⊢
  have hinc : 0 < inc := Nat.pos_of_ne_zero hb.2
  omega

@[grind ←]
theorem injective_contiguous1D {len n : Nat} (off : Nat) (h : off + len ≤ n) :
    (contiguous1D (len := len) (n := n) off h).Injective := by
  simpa [contiguous1D] using
    (injective_strided1D (len := len) (n := n) off 1 (by omega))

@[grind ←]
theorem injective_const_singleton {dst : HTuple Nat q} (x : HTuple Nat q)
    (hx : x <ₑ dst := by get_elem_tactic) :
    (const h(1) dst x hx).Injective := by
  intro ⟨.leaf i, hi⟩ ⟨.leaf j, hj⟩ _h
  apply FinHTuple.ext
  simp at hi hj ⊢
  omega
