...
/-- The full enumeration of `I` by `fromIdx` over all of `Fin n` is complete: it lists every
element of `I` (provided `n < 2 ^ 64`). -/
theorem enum_complete [DecidableEq I] (h : n < 2 ^ 64) :
    (Finset.univ : Finset I) = (List.ofFn (fun i : Fin n => fromIdx (Fin.toIdx i))).toFinset := by
  symm
  rw [Finset.eq_univ_iff_forall]
  intro x
  rw [List.mem_toFinset, List.mem_ofFn]
  exact ⟨(toIdx x).toFin, by rw [Idx.toIdx_toFin]; exact IndexType.left_inv h x⟩
