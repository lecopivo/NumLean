...
class SetElem (coll : Type u) (idx : Type v) (elem : outParam (Type w))
  ...
  setElem (xs : coll) (i : idx) (v : elem) (h : valid xs i := by get_elem_tactic) : coll
...
instance {cont elem dom} [SetElem cont Nat elem dom] :
    SetElem cont (Fin n) elem fun xs i => dom xs i where
  setElem xs i x h := setElem xs i.1 x (by grind)
  setElem_valid := by
    intros
    exact SetElem.setElem_valid (coll := cont) (idx := Nat)

@[simp]
theorem setElem_fin [SetElem cont Nat elem dom] (xs : cont) (i : Fin n) (x : elem) (h : dom xs i) :
  setElem xs i x h = setElem xs i.1 x h := by rfl

-- attribute [simp]
--   LawfulSetElem.getElem_setElem_eq
--   LawfulSetElem.getElem_setElem_neq
