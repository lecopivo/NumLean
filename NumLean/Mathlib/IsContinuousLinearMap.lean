...
open NumLean IsContinuousLinearMap ContinuousLinearMap
...
:=
  by_morphism ((mk' R f hf) + (mk' R g hg))
  (by simp)
...
-- @[fun_prop]
-- theorem NumLean.IsContinuousLinearMapMap.isLinearMap (f : X → Y) (hf : IsContinuousLinearMap R f) :
--     IsLinearMap R f := hf.1

@[fun_prop]
theorem IsContinuousLinearMap.continuous (f : X → Y) (hf : IsContinuousLinearMap R f) :
    Continuous f := hf.2
...
@[fun_prop]
theorem IsContinuousLinearMap.differentiable (f : X → Y) (hf : IsContinuousLinearMap K f) :
...
@[fun_prop]
theorem IsContinuousLinearMap.differentiableAt (x : X) (f : X → Y) (hf : IsContinuousLinearMap K f) :
