...
def validateExternInPath (path : String) : Except String Unit := do
  if path.startsWith "/" || (path.splitOn "/" |>.any (· == "..")) then
    throw "extern_in path must be relative to `c/` and must not contain `..`"
  if !path.endsWith ".c" then
    throw "extern_in path must end in `.c`"
...
def ensureExclusiveArrayLines (a : ArrayParam) : Except String (Array String) := do
  ...
  let some copyFn := arrayCopyFn? a.elem
    | throw s!"extern_in error: unsupported mutable array element type `{a.elem.render}` for parameter `{a.name}`; cannot generate a Lean ownership-safe copy before exposing a mutable pointer"
  return #[
...
partial def wrapperReturnExpr (v : Val) : Except String (String × Array String) :=
  ...
  | .tuple fields =>
      if fields.size != 2 then
        throw s!"extern_in error: unsupported return type tuple arity {fields.size}; only scalar returns, scalar-array returns, and pairs of scalar arrays are supported"
      else do
        ...
        return ("_ret", lines)
  | _ => throw s!"extern_in error: unsupported return shape `{valKind v}`; expected scalar, scalar array, or pair of scalar arrays"
...
def wrapperFor (decl : LCNF.Decl .pure) (fn : CFunction) (ffiName : String) : Except String String := do
  ...
  for p in decl.params do
    ...
    if let some k := scalarKindOfType? p.type then
      ...
      throw s!"extern_in error: unsupported parameter type for `{p.binderName}`: `{p.type}`; supported extern_in ABI types are USize, Int32, Int64, Float, Float32, FloatArray, Float32Array, Int32Array, Int64Array, and USizeArray; Nat and Int are intentionally rejected at the raw C ABI boundary"
  ...
  let (retTy, callLines, retExpr) ←
    ...
    | .scalar .. =>
        let some k := resultKind? | throw s!"extern_in error: unsupported scalar return type `{resultType}`; supported scalar returns are USize, Int32, Int64, Float, and Float32; Nat and Int are intentionally rejected at the raw C ABI boundary"
    ...
    | _ => throw s!"extern_in error: unsupported return shape `{valKind fn.retVal}`; expected scalar, scalar array, or pair of scalar arrays"
...
def writeExternInFile (relPath : String) (content : String) : CoreM System.FilePath := do
  ...
  let some parent := path.parent | throwError "invalid extern_in path `{relPath}`"
  IO.FS.createDirAll parent
  IO.FS.writeFile path content
  return path

syntax (name := Lean.Parser.Attr.extern_in) "extern_in " str : attr

initialize
  registerBuiltinAttribute {
    name := .str .anonymous "extern_in"
    descr := "compile a supported Lean declaration to raw C in c/<path> and attach an extern wrapper"
    applicationTime := AttributeApplicationTime.afterCompilation
    add := fun declName stx kind => do
      unless kind == AttributeKind.global do throwAttrMustBeGlobal `extern_in kind
      let some relPath := stx[1].isStrLit? | throwErrorAt stx "extern_in expects a string literal path"
      ...
      trace[NumLean.Meta.CCompiler.extern_in] "generated extern_in C for `{declName}` at `c/{relPath}`:\n{c}"
      ...
      | .ok env => setEnv env
      | .error e => throwError e
      logInfo m!"wrote C extern implementation to {path}"
    erase := fun _ => throwError "cannot erase extern_in"
  }
